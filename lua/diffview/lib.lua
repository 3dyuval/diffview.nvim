...
local src = lazy.require("diffview.adapters") ---@module "diffview.adapters"
...
function M.diffview_open(args)
  ...
  local force_file_adapter = argo:get_flag("no-index")

  -- When using --no-index, all args are file paths (no rev_arg)
  local path_args
  if force_file_adapter then
    path_args = argo.args -- All args are paths when --no-index is used
    rev_arg = nil -- No revision concept for file adapter
  else
    path_args = argo.post_args -- Normal VCS mode: paths come after rev
  end

  local err, adapter = src.get_adapter({
    force_file_adapter = force_file_adapter,
    cmd_ctx = {
      path_args = path_args,
